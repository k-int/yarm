...
class GOKbTextUtils {
  ...
  public static String simpleComponentHash(String s) {
    def component_name = s ?: "";
    component_name = Normalizer.normalize(component_name, Normalizer.Form.NFD)
    component_name = component_name.replaceAll("\\p{InCombiningDiacriticalMarks}+","").toLowerCase();
    return MessageDigest.getInstance("MD5").digest(component_name.bytes).encodeHex().toString()
  }
